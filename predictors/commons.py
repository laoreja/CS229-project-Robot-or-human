...
def prepareTrainData():
    dfFeatures = pd.read_csv("./features/all_feat.csv").drop(
        ["Unnamed: 0"],
        axis=1,
    )
...
def printSubmission(classifier, X_train, y_train, name):
    ...
    common = dfFeatures.merge(dfTest, on='bidder_id')
    X_test = np.array(common.drop(['bidder_id'], axis=1))
    prediction = classifier.predict(X_test)
    predictionDf = pd.DataFrame(data={"outcome": prediction})
    pd.concat([X_test, predictionDf]).to_csv("submissions/{}.csv", index=False)
